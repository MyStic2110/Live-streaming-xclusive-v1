...
/**
 * Bootstraps the database tables, seeds initial compliance frameworks, and triggers dynamic background audits.
 * @param {object} client - PostgreSQL client instance
 */
export const bootstrapDB = async (client) => {
  // 1. Initialize schema and static JSON seeds synchronously to ensure DB consistency
  await initializeTablesAndSeeds(client);

  // 2. Trigger dynamic Python compliance scanners and SRE analyzers asynchronously in the background.
  // This ensures the Node.js Express server boots instantly and does not block on startup.
  runScannersInBackground(client).catch(err => {
    logger.error(`[DATABASE] ❌ Background compliance scans failed to start: ${err.message}`);
  });
};

const initializeTablesAndSeeds = async (client) => {
  try {
    ...
    // Sync NIST subcategories from json file
    logger.info('[DATABASE] ⏳ Syncing NIST AI RMF Core subcategories...');
    const nistJsonPath = path.join(__dirname, '..', 'config', 'nist-rmf-core.json');
    ...
    // Sync OWASP subcategories from json file
    logger.info('[DATABASE] ⏳ Syncing OWASP Top 10 for LLM subcategories...');
    const owaspJsonPath = path.join(__dirname, '..', 'config', 'owasp-llm-core.json');
...
const runScannersInBackground = async (client) => {
  const isWindows = process.platform === 'win32';
  const pythonPath = isWindows
    ? path.join(__dirname, '..', '..', '..', 'python-agent', 'venv', 'Scripts', 'python.exe')
    : path.join(__dirname, '..', '..', '..', 'python-agent', 'venv', 'bin', 'python');
  
  const scopeScriptPath = path.join(__dirname, '..', '..', '..', 'python-agent', 'run_scope_analyzer.py');
  const scannerScriptPath = path.join(__dirname, '..', '..', '..', 'python-agent', 'agents', 'nist', 'scanner.py');

  // 1. Dynamic Agent Analysis (Scope Compliance Analyzer)
  try {
    if (fs.existsSync(pythonPath) && fs.existsSync(scopeScriptPath)) {
      logger.info('[DATABASE] ⏳ Running dynamic Agent Scope Compliance analysis in background...');
      const { stdout, stderr } = await execPromise(`"${pythonPath}" "${scopeScriptPath}"`);
      if (stderr) {
      ...
      logger.warn(`[DATABASE] ⚠️ Python executable or analyzer script not found. Dynamic agent analysis skipped.`);
    }
  } catch (analysisError) {
    logger.error(`[DATABASE] ❌ Dynamic Agent analysis failed: ${analysisError.message}`);
  }

  // 2. Dynamic Agent Compliance Scan
  try {
    if (fs.existsSync(pythonPath) && fs.existsSync(scannerScriptPath)) {
      logger.info('[DATABASE] ⏳ Running dynamic Agent Compliance scan in background...');
      const { stdout, stderr } = await execPromise(`"${pythonPath}" "${scannerScriptPath}"`);
      if (stderr) {
      ...
      logger.warn(`[DATABASE] ⚠️ Python executable or scanner script not found. Dynamic compliance scan skipped.`);
    }
  } catch (scanError) {
